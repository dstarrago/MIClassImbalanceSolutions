 ...
 * collective MI assumption application.
 *
 * @author Danel
 */
public class PropMICollective extends SingleClassifierEnhancer {
...
//    bag
    double labelAggregation = 0;
    for (int i = 0; i < bag.numInstances(); i++) {
      Instance x = bag.get(i);
      labelAggregation += m_Classifier.classifyInstance(x);
    }
    labelAggregation /= bag.numInstances();
    if (positiveLabel > 0)
      return (labelAggregation > 0.5)?positiveLabel:1-positiveLabel;
    else
      return (labelAggregation < 0.5)?positiveLabel:1-positiveLabel;
  }
  ...
  public static void main(String[] argv) {
    runClassifier(new PropMICollective(), argv);
  }
