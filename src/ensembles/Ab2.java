 ...
 */
  ...
  /**
   * Store misclassification autoCosts for each class
   */
  ...
  protected void buildClassifierUsingResampling(Instances data)
    ...
    for (m_NumIterationsPerformed = 0; m_NumIterationsPerformed < m_Classifiers.length;
      ...
      m_Alphas[m_NumIterationsPerformed] = Math.log(sumIfRight / sumIfWrong) / 2;
      ...
      for (int i = 0; i < training.size(); i++) {
        ...
        if (Utils.eq(prediction, instance.classValue())) {
          instance.setWeight(instance.weight() * costs[(int)instance.classValue()] *
                  Math.exp( - m_Alphas[m_NumIterationsPerformed]) / Zeta);
        } else {
          instance.setWeight(instance.weight() * costs[(int)instance.classValue()] *
                  Math.exp( + m_Alphas[m_NumIterationsPerformed]) / Zeta);
        }
