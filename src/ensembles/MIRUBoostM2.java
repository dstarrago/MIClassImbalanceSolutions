...
public class MIRUBoostM2
   ...
   */
  private boolean negBootstrap = true;
  ...
  protected void buildClassifierUsingResampling(Instances data) 
    ...
    for (m_NumIterationsPerformed = 0; m_NumIterationsPerformed < m_Classifiers.length; 
      ...
      if (m_Debug) {
        String msg = "Initial IR = %f    Final IR = %f    numPos = %d    numNegAfterProcessing = %d";
        System.out.println(String.format(msg, (double)numNeg/numPos,
                (double)numNegAfterProcessing/numPos, numPos, numNegAfterProcessing));
      }

      if (negBootstrap) {
        DNIndexes = bootstrapIndexes(numNegAfterProcessing, negatives);
      } else {
        DNIndexes = subsampleIndexes(numNegAfterProcessing, negatives);
      }
      ...
      trainData = new Instances(training, subsampledIndexes.size());
      for (int i = 0; i < subsampledIndexes.size(); i++) {
        if (subsampledIndexes.get(i) == null) continue;
        int index = subsampledIndexes.get(i).intValue();
        Instance X = (Instance)training.get(index).copy();
        X.setWeight(W[index] / sumOfWeights);
        trainData.add(X);
      }
     ...
     */
    private ArrayList<Integer> bootstrapIndexes(int n, ArrayList<Integer> D) {
  ...
  public static void main(String [] argv) {
    runClassifier(new MIRUBoostM2(), argv);
  }
}
