 ...
 */
  ...
  /**
   * Store the fourth power of misclassification costs for each class
   */
  private double[] costs4;
  ...
  protected void buildClassifierUsingResampling(Instances data)
    ...
    for (m_NumIterationsPerformed = 0; m_NumIterationsPerformed < m_Classifiers.length;
      ...
      for (int i = 0; i < training.size(); i++) {
        ...
        sum += costs2[(int)instance.classValue()] * instance.weight();
        if (Utils.eq(prediction, instance.classValue())) {
          sumIfRight += costs4[(int)instance.classValue()] * instance.weight();
        } else {
          sumIfWrong += costs4[(int)instance.classValue()] * instance.weight();
        }
      }
      m_Alphas[m_NumIterationsPerformed] = 
        Math.log((sum + sumIfRight - sumIfWrong) / (sum - sumIfRight + sumIfWrong)) / 2;
      ...
      for (int i = 0; i < training.size(); i++) {
        Instance instance = training.get(i);
        if (Utils.eq(prediction, instance.classValue())) {
          Zeta += costs2[(int)instance.classValue()] * instance.weight()
                  * Math.exp( - m_Alphas[m_NumIterationsPerformed] *
                  costs2[(int)instance.classValue()]);
        } else {
          Zeta += costs2[(int)instance.classValue()] * instance.weight()
                  * Math.exp( + m_Alphas[m_NumIterationsPerformed] *
                  costs2[(int)instance.classValue()]);
      ...
      for (int i = 0; i < training.size(); i++) {
        Instance instance = training.get(i);
        if (Utils.eq(prediction, instance.classValue())) {
          instance.setWeight(instance.weight() * costs2[(int)instance.classValue()] *
                  Math.exp( - m_Alphas[m_NumIterationsPerformed] *
                  costs2[(int)instance.classValue()]) / Zeta);
        } else {
          instance.setWeight(instance.weight() * costs2[(int)instance.classValue()] *
                  Math.exp( + m_Alphas[m_NumIterationsPerformed] *
                  costs2[(int)instance.classValue()]) / Zeta);
