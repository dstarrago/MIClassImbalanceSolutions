...
//
//  /**
//   * Cluster directory: InstIndex (in the list of clusters) of the cluster
...
//  private int[] clusterDir;
   ...
   * positive example. The first InstIndex is for the bag and the second for
...
//  private double[][] diverseDensityTable;
  ...
  /**
   * Stores the probability of each instance in each positive example to be selected.
   * This is the probability to be a positives instance calculated by a diverse
   * density function. The first InstIndex is for the bag and the second for instances
   * inside the bag. Probabilities are normalized so the sum of probabilities
   * corresponding to all the instances inside one bag equals 1.
   *
   */
...
//  private int numNNInforMin;        // K3
  private int numNNInterpol;        // K4
//  private int numNNAttrVoting;      // K4

  public MismoteDataManager(Instances data, int posClassLabel, int numNNInterpol) {
    this.data = data;
    this.posClassLabel = posClassLabel;
    this.numNNInterpol = numNNInterpol;
...
//    linkSampleToCluster();
    computeKNN();
    //evaluateDD();
    evaluateSelectionProbs();
//    evaluateBigSelectionProbs();
//    if (hasNominalAttrs()) computeKNN();
  }

  public MismoteDataManager(Instances data) {
    this(data, needToBeCalculated, defaultnumNNInterpol);
  }
...
//  }
  ...
  private void evaluateSelectionProbsFormer() {
    selectionProbs = new double[positives.size()][];
    ...
    for (int i = 0; i < positives.size(); i++) {
      ...
      selectionProbs[i] = new double[bagSize];
      for (int j = 0; j < bagSize; j++) {
        Instance x = bag.relationalValue(1).get(j);
//        selectionProbs[i][j] = DDFunction.noisyOr(x);
//        selectionProbs[i][j] = DDFunction.noisyOrNLL(x);
//        selectionProbs[i][j] = DDFunction.noisyOrNLL_XP(x);
        selectionProbs[i][j] = DDFunction.mostLikelyCause(x);
//        selectionProbs[i][j] = DDFunction.mostLikelyCauseNLL(x);
      }
      System.out.print("P" + (i+1) + ":  ");
      for (int j = 0; j < bagSize; j++) {
        System.out.print(selectionProbs[i][j] + "    ");
      }
      double nsum = Utils.sum(selectionProbs[i]);
      System.out.print(" (" + nsum + ") --> ");
      if (nsum == 0) {
        Arrays.fill(selectionProbs[i], 1.0 / bagSize);
      } else {
        Utils.normalize(selectionProbs[i], nsum);
        for (int j = 0; j < bagSize; j++) {
          System.out.print(selectionProbs[i][j] + "    ");
        }
        System.out.println();
  ...
  private void evaluateSelectionProbs() {
    boolean debug = false;
    selectionProbs = new double[positives.size()][];
    DDFunction = new DiverseDensity(data, posClassLabel);
    for (int i = 0; i < positives.size(); i++) {
      ...
      int bagSize = bag.relationalValue(1).size();
      selectionProbs[i] = new double[bagSize];
      double[] dd = new double[bagSize];
      for (int j = 0; j < bagSize; j++) {
        Instance x = bag.relationalValue(1).get(j);
//        dd[j] = DDFunction.noisyOr(x);
//        dd[j] = DDFunction.noisyOrNLL(x);
//        dd[j] = DDFunction.noisyOrNLL_XP(x);
        dd[j] = DDFunction.mostLikelyCause(x);
//        dd[j] = DDFunction.mostLikelyCauseNLL(x);
//        dd[j] = DDFunction.mostLikelyCauseOWA(x);
//        dd[j] = DDFunction.fuzzy(x);
      }
      double ddSum = Utils.sum(dd);
      if (ddSum == 0) {
        double prob = 1.0 / bagSize;
        Arrays.fill(selectionProbs[i], prob);
        if (debug) System.out.print("Equal probabilities " + prob);
      } else {
        if (debug) System.out.print("P" + (i+1) + ":  ");
        Utils.normalize(dd, ddSum);
        for (int j = 0; j < bagSize; j++) {
          double ddExp = Math.exp(dd[j]);
          if (Utils.eq(ddExp, 1)) {
            selectionProbs[i][j] = 0;
          } else {
            selectionProbs[i][j] = ddExp;
          }
          if (debug) System.out.print(selectionProbs[i][j] + "    ");
        }
        double nsum = Utils.sum(selectionProbs[i]);
        if (debug) System.out.print(" (" + nsum + ") --> ");
        Utils.normalize(selectionProbs[i], nsum);
        if (debug)
        for (int j = 0; j < bagSize; j++) {
          System.out.print(selectionProbs[i][j] + "    ");
  ...
  private void evaluateBigSelectionProbs() {
    selectionProbs = new double[positives.size()][];
    DDFunction = new DiverseDensity(data, posClassLabel);
    for (int i = 0; i < positives.size(); i++) {
      Instance bag = data.get(positives.get(i));
      int bagSize = bag.relationalValue(1).size();
      selectionProbs[i] = new double[bagSize];
      BigDecimal[] dd = new BigDecimal[bagSize];
      for (int j = 0; j < bagSize; j++) {
        Instance x = bag.relationalValue(1).get(j);
        dd[j] = DDFunction.bigNoisyOr(x);
      }
      BigDecimal sum = new BigDecimal(0, MathContext.DECIMAL32);
      for (int j = 0; j < bagSize; j++) {
        sum = sum.add(dd[j], MathContext.DECIMAL32);
      }
      if (sum.compareTo(new BigDecimal(0, MathContext.DECIMAL32)) == 0) {
        Arrays.fill(selectionProbs[i], 1.0 / bagSize);
      } else {
        System.out.print("P" + (i+1) + ":  ");
        for (int j = 0; j < bagSize; j++) {
          BigDecimal Q = dd[j].divide(sum, MathContext.DECIMAL32);
          double ddExp = Math.exp(Q.doubleValue());
          if (Utils.eq(ddExp, 1)) {
            selectionProbs[i][j] = 0;
          } else {
            selectionProbs[i][j] = ddExp;
...
////        diverseDensityTable[i][j] = DDFunction.noisyOrNLL(x);
////        diverseDensityTable[i][j] = DDFunction.mostLikelyCause(x);
//        diverseDensityTable[i][j] = DDFunction.mostLikelyCauseNLL(x);
//      }
//    }
//  }
   ...
   *
   * @param posIndex InstIndex of the given positive example in the list of postive
   * examples indexes.
...
// *
// * @param c1Indes InstIndex of the given positive example in the positive example list
...
//  }
  ...
  /**
   * Gets the number of minority neighbors used for interpolation.
   * It is a parameters of the algorithm.
   */
  public int numNNInterpol() {
    return numNNInterpol;
  }

  /**
   * Sets the number of minority neighbors used for intepolation.
   *
   * @param K the new value.
   */
  public void setNumNNInterpol(int K) {
    numNNInterpol = K;
  }

  /**
   * Gets the InstIndex of the positive class label
   */
  ...
  /**
   * Gets the size of the list of positive example indexes in the training set.
   */
  public int posCount() {
  ...
  /**
   * Gets the size of the list of negative example indexes in the training set.
   */
  public int negCount() {
...
//  /**
//   * Gets the i-th InstIndex in the list of positive examples.
//   *
...
//  }
   ...
   *
   * @param posBagIndex InstIndex in the list of positive examples of the bag
   * containing the instance.
   * @param instIndex the InstIndex of the instance inside the bag.
   *
...
//  }
   ...
   *
   * @param i InstIndex of the example in the list of positive examples.
   *
...
//  }
  ...
  /**
   * Randomly select from a given bag one instance with high probability to be
   * positive.
   */
  public Instance samplePosInstance(int posBagIndex, Random random) {
    double u = random.nextDouble();
    int InstIndex = 0;
    double prob = selectionProbs[posBagIndex][InstIndex];
    while (u >= prob) {
      prob += selectionProbs[posBagIndex][++InstIndex];
    }
    return posBag(posBagIndex).relationalValue(1).get(InstIndex);
  }

  public int samplePosInstanceIndex(int posBagIndex, Random random) {
    double u = random.nextDouble();
    int InstIndex = 0;
    double prob = selectionProbs[posBagIndex][InstIndex];
    while (u >= prob) {
  ...
  public void testProbs(Random random) {
    int T = 1000;
    System.out.println();
    System.out.println("Testing probabilities T = " + T);
    System.out.println();
    double[][] output = new double[posCount()][];
    for (int i = 0; i < posCount(); i++) {
      System.out.print("P" + (i+1) + ":  ");
      Instance bag = posBag(i);
      int bagSize = bag.relationalValue(1).size();
      output[i] = new double[bagSize];
      for (int t = 0; t < T; t++) {
        int selIndex = samplePosInstanceIndex(i, random);
        output[i][selIndex]++;
      }
      for (int j = 0; j < bagSize; j++) {
        output[i][j] /= T;
