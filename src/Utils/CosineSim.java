...
/**
 * Class to calculate cosine similarity between instances.
 *
 * @author Danel
 */
public class CosineSim extends InstanceLevelDistance {

  /**
   * Calculate the cosine similarity between two instance.
   * 
  ...
  public double measure(Instance a, Instance b) {
    if (a instanceof SparseInstance)        // Asumo que b tambien es SparseInstance
      return sparseCosSimilarity((SparseInstance)a, (SparseInstance)b);
    else
      return denseCosSimilarity((DenseInstance)a, (DenseInstance)b);
  }

  private static double sparseCosSimilarity(SparseInstance a, SparseInstance b) {
    double ab = 0, a2 = 0, b2 = 0;
    for (int i = 0; i < a.numValues(); i++) {
      if (a.index(i) == 0 || a.index(i) == a.classIndex()) continue;    // a.classIndex() ¿=? a.numAttributes() - 1
  ...
  private static double denseCosSimilarity(DenseInstance a, DenseInstance b) {
    double ab = 0, a2 = 0, b2 = 0;
